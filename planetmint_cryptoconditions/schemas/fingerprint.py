...
class ZenroomFingerprintContents(Sequence):
    componentType = NamedTypes(
        NamedType("script", OctetString().subtype(implicitTag=Tag(tagClassContext, tagFormatSimple, 0))),
    )


class PrefixFingerprintContents(Sequence):
    componentType = NamedTypes(
        NamedType("prefix", OctetString().subtype(implicitTag=Tag(tagClassContext, tagFormatSimple, 0))),
        NamedType(
            "maxMessageLength",
            Integer()
        ...
        NamedType("subcondition", Condition().subtype(implicitTag=Tag(tagClassContext, tagFormatConstructed, 2))),
    )


class RsaFingerprintContents(Sequence):
    componentType = NamedTypes(
        NamedType("modulus", OctetString().subtype(implicitTag=Tag(tagClassContext, tagFormatSimple, 0))),
    )
...
class ThresholdFingerprintContents(Sequence):
    componentType = NamedTypes(
        NamedType(
            "threshold",
            Integer()
            .subtype(subtypeSpec=ValueRangeConstraint(1, 65535))
            .subtype(implicitTag=Tag(tagClassContext, tagFormatSimple, 0)),
        ),
        NamedType(
            "subconditions",
            SetOf(componentType=Condition()).subtype(implicitTag=Tag(tagClassContext, tagFormatSimple, 1)),
        ),
    )
